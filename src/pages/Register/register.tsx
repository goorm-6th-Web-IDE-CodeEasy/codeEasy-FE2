...
const Register = () => {
  ...
  const checkAvailability = async (value, type) => {
    ...
    try {
      const response = await axios.get(
        `http://localhost:8080/api/${type}-check`,
        {
          params: { [type]: value },
        }
      );
      ...
      if (!available) {
        alert(
          `${type === "username" ? "아이디" : "닉네임"}가 이미 사용 중입니다.`
        );
      }
    } catch (error) {
  ...
  const handleSubmit = async (event) => {
    ...
    const { usernameAvailable, nicknameAvailable, emailVerified } =
      availability;
    ...
    try {
      ...
      alert(
        `회원가입에 실패했습니다: ${error.response ? error.response.data.message : error.message}`
      );
    }
  };
